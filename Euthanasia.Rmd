...
treatment_index <- which(names(train) == "treatment")
```
...
tree = tree(as.factor(treatment) ~ ., data = train)
summary(tree)
...
test_tree = predict(tree_cv$finalModel, newdata = test,type = "class")

confusionMatrix(test_tree, test$Treatment)
```
...
```{r}
pred = predict(knn, testNum[,-treatment_index])
confusionMatrix(pred, testNum$treatment)
```
...
```{r}
nb_test <- test[, -treatment_index]
pred = predict(nb, nb_test) #predict on test set

confusionMatrix(pred, test[, treatment_index]) #confusion matrix to check accuracy of model
```
